...
OPENR, 1, 'Radiation_Model/atomic_data/balances/fe.bal'
...
szfilename = STRCOMPRESS( STRING( 'Z', szZ,'.txt' ), /REMOVE_ALL )
...
WHILE ( NOT EOF(1) ) DO BEGIN
    ...
    READF, 1, time, Te, Teff, temp9, temp12, temp15, temp19, temp24
    T[i] = Te
    I1[i] = temp9
    I2[i] = temp12
    I3[i] = temp15
...
PLOT, Teq, I1eq, xrange=[5.0,8.0], yrange=[-10.0,1.0], xtitle='Log!l10!n T (K)', ytitle='Log!l10!n Population Fraction', title='Ionisation balance comparison', xth=3, yth=3, linestyle=1, th=3, charsize=1, charth=3
...
y = MAX( I1, max_index )
x = T[max_index]
IF x LT 8.0 THEN XYOUTS, x, y, 'IX', charsize=1, charth=3

y = MAX( I2, max_index )
x = T[max_index]
IF x LT 8.0 THEN XYOUTS, x, y, 'XII', charsize=1, charth=3

y = MAX( I3, max_index )
x = T[max_index]
IF x LT 8.0 THEN XYOUTS, x, y, 'XV', charsize=1, charth=3

y = MAX( I4, max_index )
x = T[max_index]
IF x LT 8.0 THEN XYOUTS, x, y, 'XIX', charsize=1, charth=3

y = MAX( I5, max_index )
x = T[max_index]
IF x LT 8.0 THEN XYOUTS, x, y, 'XXIV', charsize=1, charth=3
